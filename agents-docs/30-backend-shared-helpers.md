...
- `deliverInvitationEmail` in [`../api/_shared/invitation-email.js`](../api/_shared/invitation-email.js) — use this for Auth invite delivery so invite flows can fall back to Brevo while keeping Supabase as the source of hashed invite tokens
...
- Invite email delivery should treat Supabase Auth as the canonical token issuer, but if `inviteUserByEmail(...)` fails, fall back to `auth.admin.generateLink(...)` plus Brevo delivery through the shared invitation-email helper instead of failing hard.
