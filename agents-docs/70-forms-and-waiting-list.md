...
- Internal session reports follow the same published-content rule: resolve `metadata.published_form_schema` plus active shared blocks, materialize the resolved schema into `form_submissions.metadata.form_schema_snapshot`, and snapshot visibility/alert rules. Never fill a report from the editable draft `forms.form_schema` or silently fall back to it.
...
- Do not enforce a single waiting-list intake form per org. Multiple active `waiting_list_intake` forms are intentional: orgs may personalize intake by audience, service, campaign, referral source, promotion, language, or priority segment. Safety should come from clear naming, selection UX, published-version snapshots, and inactive-state handling, not from uniqueness constraints.
...
- Input-local actions on schema-rendered text fields must use `SectionedFormRenderer`'s `renderQuestionInputAction(question)` hook; do not duplicate the shared question renderer to add an icon or picker beside an input.
...
- The waiting-list page's `pending_form` filter is read-only and comes from pending `form_submissions` whose metadata workflow is `waiting_list_intake` **and** whose linked form has `form_usage='waiting_list_intake'`. It includes both student-backed and client-profile-only subjects; it must not treat arbitrary pending forms as waiting-list intake records or expose scheduling/status actions before submission creates a real `waiting_list_entries` row.
