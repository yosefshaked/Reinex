...
- Shared ledger presentation helper: [`../src/features/finance/utils/ledgerPresentation.js`](../src/features/finance/utils/ledgerPresentation.js) — normalize ledger note text for user-facing Hebrew labels (for example reversal notes) before rendering in billing tables.
- Session-scoped Sheet draft storage: [`../src/features/finance/utils/sheetDraftStorage.js`](../src/features/finance/utils/sheetDraftStorage.js) — use `sessionStorage` for finance side-panel drafts that should survive accidental close/reopen within the current browser session, but clear them after a successful save.
...
- Cancelled `hmo_invoice_batch_items` are retained for audit. Claim-line reuse must ignore cancelled items and treat only active item statuses (`draft`, `submitted`, `acknowledged`, `partially_paid`, `paid`, `disputed`) from active batches as blocking.
...
- **Pay rules spec (owner-approved, 2026-09-17):** [`../implementations/business-process/payroll-scenarios.md`](../implementations/business-process/payroll-scenarios.md), with the technical plan in [`payroll-prerequisites.md`](../implementations/business-process/payroll-prerequisites.md). Every scenario ID (`PAY-A1` … `PAY-K7`) is in [`../test/payroll-rules.test.js`](../test/payroll-rules.test.js): rules that exist are real tests, rules not yet built are `it.todo`. When you build a rule, turn its todo into a real test first. The key decisions:
  - Rates are dated in `RateHistory`, with a `pay_basis` column: `lesson_hourly` / `lesson_flat` (per service), `attendance_hourly`, `monthly_salary`, `leave_day`.
    ...
    - One dialog adds every kind of rate: the office picks what the rate is (per service / hourly / monthly / leave day) and the fields follow. A lesson rate always belongs to a service; the other kinds never have one.
    - **Pay is additive across kinds** (2026-09-23). `api/payroll` adds every part the employee has rates for — lesson earnings, office hours × `attendance_hourly`, a pro-rated `monthly_salary` — through `combinePayComponents` in [`../api/_shared/employee-finance.js`](../api/_shared/employee-finance.js). `Employees.payroll_model` is a label in the response, never a gate. An instructor who also does stable work is paid for both; use `hasRateKind(rows, {employeeId, payBasis})` to ask whether a part exists.
    - Paid leave is added on top **except** when a `monthly_salary` rate exists, because that salary is pro-rated over the month's working days and already covers the leave day. The leave-day value follows the same rule.
    - The panel groups the cards by what pays them ("לפי מפגש" / "שכר עבודה"). A service the employee has no capability for is tagged "שירות לא משויך"; nothing is tagged unused, because every kind the employee has a rate for is paid.
    ...
    - **A capability carries no money.** `api/instructors` ignores `base_rate` and `compensation_input`, the capability dialogs have no rate field, and the `instructor_service_capabilities` → `RateHistory` trigger is gone (patch 2026-09-23). It used to store a per-session amount as `base_rate = amount x 60 / duration` and copy that into `RateHistory` as `lesson_hourly`, so pay changed whenever a lesson's length did. Never reintroduce a second writer: `api/employee-rates` is the only one.
    - The employee-level mirror (`mirror_employee_rates_to_rate_history`) stays for now: it copies `current_rate` / `monthly_salary_amount` / `leave_fixed_day_rate` verbatim, with no arithmetic. Those columns are still read for display only, and get dropped in a later step once the rebuilt rates have been reviewed.
  - **Attendance separates lesson time from other work.** `employee_attendance_records` holds `lesson_minutes` (written only by `syncInstructorAttendanceFromLessons` from the lessons the instructor is compensated for) and `other_minutes` (office work, written only by `api/employee-attendance`); `worked_minutes` is their sum, kept by a DB trigger. **Anything paying by the hour must read `resolveOtherMinutes(row)`**, never `worked_minutes` — lesson time is already paid through `lesson_earnings`. The two writers never overwrite each other, and a day keeps the bucket the other one owns (including on delete).
  - **Leave pay is not a rate for work.** It is a method on the employee (`leave_pay_method`: the legal average, a historical average, or a fixed day value), shown with the leave settings in the employee card — not in the rates screen. `RateHistory.leave_day` only carries the dated value for the `fixed_rate` method.
...
- Student roster finance-source tags are informational only. An active HMO authorization may label the student with the provider; otherwise the student is shown as regular billing. Do not treat missing HMO authorization as a scheduling/generation warning by itself.
