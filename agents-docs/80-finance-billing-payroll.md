...
- **Pay rules spec (owner-approved, 2026-09-17):** [`../implementations/business-process/payroll-scenarios.md`](../implementations/business-process/payroll-scenarios.md), with the technical plan in [`payroll-prerequisites.md`](../implementations/business-process/payroll-prerequisites.md). Every scenario ID (`PAY-A1` … `PAY-K7`) is in [`../test/payroll-rules.test.js`](../test/payroll-rules.test.js): rules that exist are real tests, rules not yet built are `it.todo`. When you build a rule, turn its todo into a real test first. The key decisions:
  - Rates are dated in `RateHistory`, with a `pay_basis` column: `lesson_hourly` / `lesson_flat` (per service), `attendance_hourly`, `monthly_salary`, `leave_day`.
    ...
    - **`api/payroll` pays an employee by exactly one model** (`payroll_model`, an if/else), so a rate of a kind that model doesn't read pays nothing. `resolveEmployeePayFields(payBasis, employee)` in [`../api/_shared/rate-history.js`](../api/_shared/rate-history.js) returns the employee fields that kind needs (`payroll_model`, or `leave_pay_method` for a leave-day rate), the dialog states the switch before saving, and `apply_pay_fields: true` on the POST applies it. Until payroll sums across kinds (P2's "rows of both kinds" end state), an employee is paid by one kind at a time.
    - The panel groups the cards by what pays them ("לפי מפגש" / "שכר עבודה"); a rate the pay model doesn't read is tagged "לא בשימוש" and a service the employee has no capability for is tagged "שירות לא משויך".
