...
- Lesson-template writes must protect instructor time slots. Overlapping templates for the same instructor/day/date range are allowed only when they represent the same group (`service_id` + exact start time + duration) and the group remains within `instructor_service_capabilities.max_students`.
