  ...
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      className={cn(
        ...
        // Cap the popup to the available viewport height (fallback 24rem) so a long list
        // (e.g. many source columns) never extends off-screen with unreachable items.
        "relative z-[60] min-w-[8rem] max-h-[var(--radix-select-content-available-height,24rem)] overflow-hidden rounded-md border bg-popover text-popover-foreground shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 origin-[--radix-select-content-transform-origin] pointer-events-auto touch-pan-y",
        position === "popper" &&
          "data-[side=bottom]:translate-y-1 data-[side=left]:-translate-x-1 data-[side=right]:translate-x-1 data-[side=top]:-translate-y-1",
        className
      )}
      // Allow scroll to work even when nested inside a modal/Dialog (react-remove-scroll)
      // The attribute below is respected by react-remove-scroll to allow scrolling this element.
      data-remove-scroll-allow
      ...
      <SelectPrimitive.Viewport
        className={cn(
          // Make the viewport the scroll container with a safe max-height.
