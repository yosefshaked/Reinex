...
const RULES = {
  ...
  UPSERT_CONFLICT_INDEX_COVERAGE: 'SQL018',
...
// SQL018: Every table registered in the upsert-conflicts validator must have a
// matching CREATE UNIQUE INDEX or named UNIQUE constraint in setup-sql. Without
// this, ON CONFLICT upserts will fail at runtime with 42P10 even though the
// JS code and SQL file individually look correct.
const KNOWN_UPSERT_CONFLICT_COLUMNS = {
  Settings: 'org_id,key',
...
function buildColPattern(cols) {
  // Match column names with or without double-quote wrapping, separated by commas
...
function validateUpsertConflictIndexCoverage() {
  for (const [table, cols] of Object.entries(KNOWN_UPSERT_CONFLICT_COLUMNS)) {
    ...
    // 1. Explicit CREATE UNIQUE INDEX on those columns
...
function main() {
  ...
  if (!sql.trim()) {
    ...
    validateUpsertConflictIndexCoverage();
    validateWarnings();
  }
