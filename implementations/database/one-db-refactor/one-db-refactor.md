...
| 24 | 2026-04-18 | Regression hardening for org-scoped upserts: fixed multiple `onConflict` targets to include `org_id` where required by composite unique indexes (`instructor_service_capabilities`, `Settings`, `grace_cancellation_requests`, `client_guardians`) and added automated guard script `lint:upsert-conflicts` to prevent recurrence. | `api/instructors/index.js`, `api/settings/index.js`, `api/calendar-attendance/index.js`, `api/_shared/client-profiles.js`, `api/students-list/index.js`, `api/waiting-list-intake/index.js`, `scripts/validate-upsert-conflicts.js`, `package.json`, `agents-docs/30-backend-shared-helpers.md`, `implementations/database/one-db-refactor/one-db-refactor.md` |
