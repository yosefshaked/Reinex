...
| **P2** | ✅ **Built** — phase 1 (2026-09-17): pay reads only `RateHistory`, with backfill and transition triggers. Phase 1b (2026-09-22): the rates screen in the employee's finance tab (`api/employee-rates`, `EmployeeRatesPanel.jsx`) with effective dates, scheduled rates, history, hourly-or-flat and back-dating warnings. **Dated rates in `RateHistory`** (owner decision, 2026-09-15). A rate is "from this date on, until the next rate change, the rate is X". Details below the table. | Otherwise a paid month can't be reproduced. |
...
| **P5** | **A trusted source of hours** for hourly and monthly staff: derived from lessons, clocked in, or entered manually. This is a decision to make. **Partly built (2026-09-22):** an attendance day now separates `lesson_minutes` (written from the lessons the instructor is paid for) from `other_minutes` (work that isn't a lesson, entered by the office), and hourly pay reads only the second, so lesson time is never paid twice. Self-entry and approval (PAY-E3) are still open. | Hourly pay is only as good as its hours. |
