...
function classifyTenantDbError(error, { resource, operation, tenantAuth } = {}) {
  ...
  if (code === 'PGRST301') {
    return {
      ...
      body: {
        ...
        message: 'Tenant API rejected tenant auth material (PGRST301). This usually means org_settings.anon_key is not the tenant anon/service key (apikey) OR the stored dedicated app_user JWT is not valid for the tenant. Verify roles: anon/service_role + app_user. See details.tenant_auth for the roles/shapes used by this request.',
        details: {
...
async function handleGet(context, tenantClient, req) {
  ...
  if (error) {
    ...
    const classified = classifyTenantDbError(error, {
      resource: 'lesson_instances',
      operation: 'select',
      tenantAuth: context.__tenantAuthDiagnostics,
    });
  ...
  if (participantsError) {
    ...
    const classified = classifyTenantDbError(participantsError, {
      resource: 'lesson_participants',
      operation: 'select',
      tenantAuth: context.__tenantAuthDiagnostics,
    });
...
async function handlePost(context, tenantClient, req) {
  ...
  if (instanceError) {
    ...
    const classified = classifyTenantDbError(instanceError, {
      resource: 'lesson_instances',
      operation: 'insert',
      tenantAuth: context.__tenantAuthDiagnostics,
    });
